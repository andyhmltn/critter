...
fn report_sections(files: &[ChangedFile], diff: &str) -> Vec<ReportSection> {
    ...
    let mut hotspots = files
        ...
        .collect::<Vec<_>>();
    hotspots.sort_by(|left, right| right.0.cmp(&left.0));
...
enum FlowState {
    Loading {
        receiver: std::sync::mpsc::Receiver<FlowEvent>,
        status: String,
        started: Instant,
    },
    Ready(FlowPlan),
...
fn start_flow_analysis(repo: &str, pr: &str, head: &str, title: &str, diff: &str) -> FlowState {
    match flow::load(repo, pr, head) {
        ...
        _ => {
            ...
            std::thread::spawn(move || {
                flow::generate_and_cache(values.0, values.1, values.2, values.3, values.4, sender)
            });
            FlowState::Loading {
                receiver,
                status: "Starting logical review analysis".to_string(),
                started: Instant::now(),
            }
        }
    }
...
fn poll_flow(app: &mut App) {
    let events = match &app.flow_state {
        FlowState::Loading { receiver, .. } => receiver.try_iter().collect::<Vec<_>>(),
        _ => return,
    };
    for event in events {
        match event {
            FlowEvent::Status(next) => {
                if let FlowState::Loading { status, .. } = &mut app.flow_state {
                    *status = next;
                }
            }
            FlowEvent::Complete(result) => {
                app.flow_state = match result {
                    Ok(plan) => FlowState::Ready(plan),
...
fn draw(app: &mut App, frame: &mut ratatui::Frame) {
    ...
    let diff_area = if app.sidebar_visible {
        ...
        let show_flow_footer = matches!(
            app.flow_state,
            FlowState::Loading { .. } | FlowState::Failed(_)
        );
            ...
            && let FlowState::Ready(plan) = &app.flow_state
        {
            ...
            let rationale_width = file_panes[0].width.saturating_sub(8) as usize;
            let items = plan.steps.iter().enumerate().map(|(index, step)| {
                let mut lines = vec![Line::styled(
                    format!("{}. {}", index + 1, step.title),
                    Style::default().add_modifier(Modifier::BOLD),
                )];
                lines.extend(
                    wrap_words(&step.rationale, rationale_width)
                        .into_iter()
                ...
                lines.push(Line::styled(
                    format!(
                        "   {} location{}",
                        step.locations.len(),
                        if step.locations.len() == 1 { "" } else { "s" }
                    ),
                    Style::default().fg(MUTED),
                ));
        ...
        match &app.flow_state {
            FlowState::Loading {
                ...
                let frames = ["✦", "✧", "⋆", "·"];
                let frame_index = (started.elapsed().as_millis() / 180) as usize % frames.len();
                frame.render_widget(
                    Paragraph::new(format!(
                        " {} {}\n Reviewing remains available · {}s",
                        frames[frame_index],
                        status,
                    ...
                    .block(
                        Block::default()
                            .title(" AI flow · working in background ")
                            .borders(Borders::ALL)
    ...
    let status = match &app.mode {
        ...
        {
            "LINE  j/k line  zz center  Enter comment  Esc back  b intelligence  / search  : command  s submit"
        }
        Mode::Browse if app.sidebar_visible => {
            "BLOCK  j/k change  V choose line  zz center  Enter comment  b intelligence  / search  : command  s submit"
        }
        Mode::Browse => {
            "BLOCK  j/k change  V choose line  zz center  Enter comment  b intelligence  / search  : command  s submit"
        }
...
fn handle_browse(app: &mut App, key: KeyEvent, workspace: &Path) {
    ...
    match key.code {
        ...
        KeyCode::Char('f') if matches!(app.focus, Focus::Files) => {
            if matches!(app.flow_state, FlowState::Ready(_)) {
